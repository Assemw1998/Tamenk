...
<div class="content-wrapper">
    ...
    <section action="" class="mt-3 content text-dark center p-3">
        <form action="{{ route('super-admin.dashboard.country-update',['id' => $country->id])}}" method="POST">
            @csrf
            <div class="form-group">
                <label for="name">Brand Name</label>
                <input type="text" class="form-control" value="{{(old('name')? old('name'): $country->name )}}" name="name" id="name" required>
            </div>
